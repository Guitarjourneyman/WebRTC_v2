...
export default function App() {
    ...
    // React 탈을 쓴 javascript use구문 사용하지않음    
    let secondPeerOn = false;
    ...
    signalingChannel.addEventListener('message', async event => {
      ...
      // Received an offer → Create and send an answer
        ...
        pendingCandidates.forEach(async candidate => {
          try {
            console.log('[Peer] Adding buffered ICE candidate:', candidate);
        ...
        if(secondPeerOn === false){
          ...
          console.log('[Peer] remotestream already exists', remoteStream);
          console.log('secondPeerOn is true');
        }
    ...
    function createPeerConnection() {
      ...
      pc.addEventListener('track', event => {
        const tmp_remoteStream = [remoteStream];
        if (!tmp_remoteStream[0]) {
          console.log('[Peer] Creating new remote stream');
          ...
          console.log('[Peer] remote stream exists');
        }
