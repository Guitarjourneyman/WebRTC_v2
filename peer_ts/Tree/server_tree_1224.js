...
io.on('connection', socket => {
    ...
    /**
 * join-broadcast + join-reconnection 통합 버전
 *
 * 클라이언트에서 아래처럼 호출한다고 가정:
 *   socket.emit('join', { room: 'room1', type: 'broadcast' })
 *   socket.emit('join', { room: 'room1', type: 'reconnection' })
 *
 * type 값으로 "broadcast" / "reconnection" 분기
 */
    socket.on('join', (payload) => {
        ...
        // --------------------------------------------
        // 3) type === 'reconnection' (기존 join-reconnection 로직)
        // --------------------------------------------
        if (type === 'reconnection') {
            // 기존 peer 가져오기 
            let peer = peers.get(setid);
            if (!peer) {
                console.log(`[Server] No existing peer found for reconnection with id: ${setid}`);
                return;
            }
    ...
    /* 기존 join-reconnection, join-broadcast 주석 처리
...
*/
    ...
    socket.on('offer', ({ to, data }) => {
        ...
        if (!targetPeer || !targetPeer.socket) {   // 없으면 여기서 drop
    ...
    function removePeer(roomid, peerid) {
        ...
        console.log(`[Server] Peer ${peerid} disconnected from room ${room}`);
        console.log('[Server] Updated broadcasters ', broadcast.broadcasters);
    }
