/*
    ...
    ** hard Reset 기능 수정중 disconnected -> failed로 옮기면서 방재접속 로직 추가
/* 
            ...
            if (pc.connectionState === 'failed') {
                ...
                if (pcType === 'recvonly') {
                    try {
                        ...
                        console.log(`[${peerId}] recvonly connection. Closing peer connection.`);
                        if (pcsRef.current[peerId]) {
                            pcsRef.current[peerId].close();
                            delete pcsRef.current[peerId];
                        ...
                        socketRef.current?.emit('join', { room: room, type: 'reconnection' });
                ...
                const pcType = pcTypesRef.current[peerId];
                // Only attempt hard reset for recvonly connections
                if (pcType === 'recvonly') {
                    console.log(`[${peerId}] recvonly connection lost. Attempting hard reset.`);
                    try {
                        if (hardResetRef.current) {
                            await hardResetRef.current(peerId);
                        } else {
                            // fallback cleanup if hardReset isn't available yet
                            if (pcsRef.current[peerId]) {
                                pcsRef.current[peerId].close();
                                delete pcsRef.current[peerId];
                                delete pcTypesRef.current[peerId];
                    ...
                    // For non-recvonly peers, perform the existing cleanup
                    console.log(`[${peerId}] Non-recvonly connection. Closing peer connection.`);
                    if (pcsRef.current[peerId]) {
                        pcsRef.current[peerId].close();
                        delete pcsRef.current[peerId];
                        delete pcTypesRef.current[peerId];
