...
function App() {
    ...
    useEffect(() =>  
        ...
        socket.on('offer', async ({ from, data }: { from: string, data: any }) => {
            ...
            // getLocalStream이 호출되지 않았을 수도 있으므로 여기서 호출
            // if (!localStreamRef.current) {
            //     console.log('[Peer]' + ' localStreamRef is null. Calling getLocalStream...');
            //     getLocalStream();
            // }
    ...
    //         return pc;
    //     }, [socket,myid, localStreamState]); // 의존성 배열이 비어있으므로 이 함수는 컴포넌트가 처음 렌더링될 때 한 번만 생성
    const createPeerConnection = (peerId : string) => {
        const configuration = { iceServers: [{ urls: 'stun:stun.l.google.com:19302' }] };
        const pc = new RTCPeerConnection(configuration);
        console.log('[Peer] createPeerConnection...');
