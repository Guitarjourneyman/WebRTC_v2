/* Recvonly 되는 지 확인 중 offerer: recvonly
    지연 시간 후 연결 확인
    비디오가 화면에 뜨지 않고 검은박스만 팝업됨
...
const socket = io(`https://192.168.0.6:8000`, { autoConnect: false });
...
function App() {
    ...
    const [localStreamState, setlocalStreamState] = useState<boolean>(false);
    ...
    const getLocalStream = useCallback(async () => {
        try {
            ...
            if (localVideoRef.current) {
                ...
                setlocalStreamState(true);
            }


            // 로컬 스트림을 추가하여 송신을 시작합니다.
            // 이 호출이 onnegotiationneeded 이벤트를 트리거합니다.
            // Object.values(pcsRef.current).forEach(pc => {
            //     localStreamRef.current!.getTracks().forEach(track => {
            //         // 이미 트랙이 있다면 교체, 없다면 추가
            //         const sender = pc.getSenders().find(s => s.track?.kind === track.kind);
            //         if (sender) {
            //             console.log('[Peer] Replacing track:');
    ...
    useEffect(() => {
        ...
        socket.on('existing-peers', (peers: Record<string, any>) => {
            ...
            peers.forEach(async (peerid: string) => {
                ...
                // console.log('[Peer] wait for localstream');
                // await waitForTaskB();
                // console.log('[Peer] localstream ready');
        ...
        socket.on('offer', async ({ from, data }: { from: string, data: any }) => {
            ...
            console.log("[Peer] answerer Pending queue: ", queue);
            if (queue) {
            ...
            // if (queue) {
            //     console.log("[Peer] pending candidates...");
            //     const addCandidatePromises = queue.map(cand => pc.addIceCandidate(new RTCIceCandidate(cand)));
            //     await Promise.all(addCandidatePromises)
            //         .then(() => console.log("[Peer] All pending candidates added successfully."))
            //         .catch(e => console.warn('[Peer] addIcecandidate failed', e));
            // }
            // 처리가 완료된 큐는 비워준다.
        ...
        socket.on('answer', async ({ from, data }: { from: string, data: any }) => {
            const pc = pcsRef.current[from];
            if (!pc) {
            ...
            // Queue 비우기: answer를 받은 후 대기 중이던 ICE 후보들을 처리
            // const queue = pendingCandRef.current[from];
            // console.log("[Peer] offerer Pending queue: ",queue);
            // if (queue) {
            //     console.log("[Peer] pending candidates...");
            //     let n = 0;
            //     for (const cand of queue) {
            //         try {
            //             console.log("[Peer] pending candidates...",n++);
            //             await pc.addIceCandidate(new RTCIceCandidate(cand));
            //         } catch (e) {
            //             console.warn('[Peer] addIcecandidate failed', e);
            //         }
        ...
        socket.on('candidate', async ({ from, data }: { from: string, data: any }) => {
            // 시간 지연
            // console.log('[Peer/Test] Waiting for 3 seconds before adding ICE candidate...');
            // await new Promise(resolve => setTimeout(resolve, 3000));
            // console.log('[Peer/Test] Waiting for 3 seconds after adding ICE candidate...');

            const pc = pcsRef.current[from];
            console.log('[Peer/Test] received ICE candidate event:');
            if (pc) {
                console.log('[Peer/Test] received ICE candidate event:', data.candidate);
    ...
    const createPeerConnection = useCallback((peerId: string): RTCPeerConnection => {
        ...
        //     console.log(`[Peer] Negotiation needed with ${peerId}`);
        //     // const offer = await pc.createOffer();
        //     // await pc.setLocalDescription(offer);
        //     // // 새로운 Offer를 상대방에게 전송
        //     // socket.emit('offer', { to: peerId, data: offer });
        ...
        return pc;
    }, [socket, myid]); // 의존성 배열이 비어있으므로 이 함수는 컴포넌트가 처음 렌더링될 때 한 번만 생성
    ...
    const sendMyVideo = useCallback(async (peerId: string) => {
        const pc = pcsRef.current[peerId];
        if (pc === null) {
        ...
        try {
            ...
            console.log(`[Peer] ${peerId}Local video stream sent.`);
    ...
    useEffect(() => {
        ...
        if (localStreamState && promiseResolverRef.current) {
            console.log("useEffect:  localStreamState가 true가 되어 Promise를 resolve합니다.");
    ...
    }, [localStreamState]); //  localStreamState 상태를 의존성 배열에 넣음
    return (
        <div style={{ padding: 16, fontFamily: "system-ui, sans-serif" }}>
            ...
            <div style={{ display: 'flex', width: 480, height: 240 }}>
                ...
                <button onClick={() => sendMyVideo(myid)} style={{ marginLeft: 8 }}>Send My Video</button>
