...
function App() {
    ...
    const iceCandidateGatheredArrayRef = useRef<RTCIceCandidate[]>([]);
    ...
    useEffect(() => {
        ...
        socketRef.current.on('existing-peers', (peers: Record<string, any>) => {
            ...
            peers.forEach(async (peerid: string) => {
                ...
                console.log(`[Peer] ICE candidates sent to ${peerid}`);
            });
        });
    ...
    const createPeerConnection = useCallback((peerId: string, type: string): RTCPeerConnection => {
        ...
        pc.onicecandidate = event => {
            ...
            if (event.candidate) {
                iceCandidateGatheredArrayRef.current.push(event.candidate);
    ...
    const sendIceCandidate = useCallback((peerId: string) => {
        const candArray = iceCandidateGatheredArrayRef.current;
        const pc = pcsRef.current[peerId];  
        ...
        iceCandidateGatheredArrayRef.current.splice(0, iceCandidateGatheredArrayRef.current.length); // 배열 초기화
